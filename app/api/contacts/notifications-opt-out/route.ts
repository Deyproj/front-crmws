...
export async function GET(request: NextRequest) {
  ...
  try {
    upstream = await fetch(`${apiBase}/contacts/notifications-opt-out${query}`, {
      headers: { 'Content-Type': 'application/json', ...forwardAuth(request) },
    });
  } catch (err) {
    console.error('[contacts/notifications-opt-out] upstream fetch failed:', err);
    return Response.json({ error: 'Backend unreachable' }, { status: 502 });
  }
  return proxyJsonResponse(upstream, 'contacts/notifications-opt-out');
}
