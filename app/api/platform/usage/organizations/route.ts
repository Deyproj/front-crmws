...
export async function GET(request: NextRequest) {
  ...
  try {
    upstream = await fetch(`${apiBase}/platform/usage/organizations`, {
      headers: { 'Content-Type': 'application/json', ...forwardAuth(request) },
    });
  } catch (err) {
    console.error('[platform/usage/organizations GET] upstream fetch failed:', err);
    return Response.json({ error: 'Backend unreachable' }, { status: 502 });
  }
  return proxyJsonResponse(upstream, 'platform/usage/organizations');
}
