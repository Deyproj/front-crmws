...
export async function PATCH(request: NextRequest) {
  ...
  try {
    upstream = await fetch(`${apiBase}/organizations/me/auto-release-conversations`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json', ...forwardAuth(request) },
      body,
    });
  } catch (err) {
    console.error('[organizations/me/auto-release-conversations PATCH] upstream fetch failed:', err);
    return Response.json({ error: 'Backend unreachable' }, { status: 502 });
  }
  return proxyJsonResponse(upstream, 'organizations/me/auto-release-conversations');
}
