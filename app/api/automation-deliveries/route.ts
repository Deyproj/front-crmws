...
export async function GET(request: NextRequest) {
  ...
  try {
    upstream = await fetch(`${apiBase}/automation-deliveries${query}`, {
      headers: { 'Content-Type': 'application/json', ...forwardAuth(request) },
    });
  } catch (err) {
    console.error('[automation-deliveries GET] upstream fetch failed:', err);
    return Response.json({ error: 'Backend unreachable' }, { status: 502 });
  }
  return proxyJsonResponse(upstream, 'automation-deliveries');
}
