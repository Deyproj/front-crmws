...
export async function GET(request: NextRequest) {
  ...
  try {
    upstream = await fetch(`${apiBase}/broadcasts/audience/preview${query}`, {
      headers: { 'Content-Type': 'application/json', ...forwardAuth(request) },
    });
  } catch (err) {
    console.error('[broadcasts/audience/preview GET] upstream fetch failed:', err);
    return Response.json({ error: 'Backend unreachable' }, { status: 502 });
  }
  return proxyJsonResponse(upstream, 'broadcasts/audience/preview');
}
