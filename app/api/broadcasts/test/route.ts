...
export async function POST(request: NextRequest) {
  ...
  try {
    upstream = await fetch(`${apiBase}/broadcasts/test`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...forwardAuth(request) },
      body,
    });
  } catch (err) {
    console.error('[broadcasts/test POST] upstream fetch failed:', err);
    return Response.json({ error: 'Backend unreachable' }, { status: 502 });
  }
  return proxyJsonResponse(upstream, 'broadcasts/test');
}
