...
export async function GET(request: NextRequest) {
  ...
  try {
    upstream = await fetch(`${apiBase}/gymsoft/reminder-rules`, {
      headers: { 'Content-Type': 'application/json', ...forwardAuth(request) },
    });
  } catch (err) {
    console.error('[gymsoft/reminder-rules GET] upstream fetch failed:', err);
    return Response.json({ error: 'Backend unreachable' }, { status: 502 });
  }
  return proxyJsonResponse(upstream, 'gymsoft/reminder-rules');
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error('[gymsoft/reminder-rules POST] upstream fetch failed:', err);
    return Response.json({ error: 'Backend unreachable' }, { status: 502 });
  }
  return proxyJsonResponse(upstream, 'gymsoft/reminder-rules');
}
