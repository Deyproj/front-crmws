...
export async function POST(request: NextRequest) {
  ...
  try {
    upstream = await fetch(`${apiBase}/gymsoft/subscriptions/sync`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...forwardAuth(request) },
    });
  } catch (err) {
    console.error('[gymsoft/subscriptions/sync POST] upstream fetch failed:', err);
    return Response.json({ error: 'Backend unreachable' }, { status: 502 });
  }
  return proxyJsonResponse(upstream, 'gymsoft/subscriptions/sync');
}
