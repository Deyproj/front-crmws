...
export async function POST(request: NextRequest) {
  ...
  try {
    upstream = await fetch(`${apiBase}/conversations/start`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...forwardAuth(request) },
      body,
    });
  } catch (err) {
    console.error('[conversations/start] upstream fetch failed:', err);
    return Response.json({ error: 'Backend unreachable' }, { status: 502 });
  }
  return proxyJsonResponse(upstream, 'conversations/start');
}
