...
export function BroadcastsView() {
  ...
  return (
    <div className="flex w-full flex-col gap-[var(--space-8)]">
      <div>
        <h1 className="text-xl font-bold text-ink">Difusión</h1>
        <p className="mt-[var(--space-3)] text-sm text-secondary">
          Envía una plantilla aprobada por Meta a los clientes con plan vigente sincronizados desde GymSoft.
        </p>
      </div>

      <section className="rounded-xl border border-border bg-surface p-[var(--space-8)]">
        <p className="text-xs font-semibold uppercase tracking-wide text-secondary">Nueva difusión</p>

        {preview && (
          <div className="mt-[var(--space-5)] flex flex-wrap gap-[var(--space-6)] rounded-lg border border-border bg-app px-[var(--space-6)] py-[var(--space-5)] text-sm">
        ...
        <form onSubmit={handleSubmit} className="mt-[var(--space-6)] flex flex-col gap-[var(--space-5)]">
          ...
          {!templatesError && templates.length === 0 && (
            <p className="text-xs text-secondary">
              No hay plantillas de difusión todavía. Créala en Meta, sincronízala en Configuración → Canal de WhatsApp y
              clasifícala como &ldquo;Difusión masiva&rdquo;.
            </p>
          )}
          {selectedTemplate && (
          ...
          <p className="text-xs text-secondary">
            ...
            Meta limita a cuántas personas distintas se les puede escribir por día ({DEFAULT_DAILY_LIMIT} para un número
            nuevo); al llegar al tope, la campaña sigue al día siguiente.
          </p>
      ...
      <section className="flex flex-col gap-[var(--space-5)]">
        ...
        {loading ? (
          ...
          <p className="text-sm text-secondary">Todavía no has creado ninguna difusión.</p>
        ) : (
                ...
                {broadcasts.map((broadcast) => (
                  <tr key={broadcast.id}>
                    ...
                    <td className="px-[var(--space-6)] py-[var(--space-4)] text-secondary">
                      ...
                      {broadcast.failedCount > 0 && <span className="text-danger"> · {broadcast.failedCount} fallaron</span>}
...
function Metric({ label, value, highlight = false }: { label: string; value: number; highlight?: boolean }) {
  return (
    <div className="flex flex-col">
      ...
      <span className={`text-lg font-bold ${highlight ? 'text-brand' : 'text-ink'}`}>{value}</span>
    </div>
  );
}
